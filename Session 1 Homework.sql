...
KD: Why the A%? rather than just "A"? 

-- "invoices" table
-- Remember: run ".schema invoices" to see the structure of the "invoices" table
KD: What does .schema invoices mean? Or have to do with? 
...
KD: At first I diid WHERE total = > 10...is = only for a single number not a string? 
...
KD: Orginally I did LIMIT 10 DESC and got an error. DESC needs to go before LIMIT 10
...
KD: Took me a while, I put where total > 5 at the end first and had AND WHERE which i now see doesn't make sense. 
...
KD: Took me a while to understand if you select the city first or order by limit 10
...
KD: I can get it to run for > 2010-01-01 but not for < than.  
...
KD: In general I find it hard to not see the headers when writting the query.  Have to remember that average = AVG(total) ect.  I find myself running other querys to see the headers and then going back
